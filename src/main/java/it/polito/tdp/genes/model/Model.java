...
import it.polito.tdp.genes.db.Edge;
import it.polito.tdp.genes.db.GenesDao;
import it.polito.tdp.genes.db.VerticiCollegati;
...
public class Model {
	...
	public void creaGrafo() {
		...
		for(Edge e : dao.getAllEdges()) {
			Graphs.addEdge(this.grafo, e.getI1(), e.getI2(), e.getD());
		}
	}
	...
	private boolean condizioneDiTerminazione(List<Integer> parziale, double soglia) {
		if(parziale.size()>0) {
			// non ci sono piÃ¹ vertici che io possa aggiungere
	...
	private boolean aggiuntaValida(int prova, List<Integer> parziale, double soglia) {
		...
		if(parziale.size()==0) {
			...
			// L'aggiunta Ã¨ valida se esiste un arco che connette l'ultimo vertice in parziale e il nuovo e se il peso dell'arco Ã¨ maggiore della soglia
